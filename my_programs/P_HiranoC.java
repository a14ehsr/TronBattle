...
public class P_HiranoC {
    ...
    static final String playerName = "HIRANO_C"; // Do not include spaces in your name.
    ...
    private int[] calc(int FACE, int x, int y) {
        int cnt = 0;
        int[][] point = new int[height + 2][width + 2];
        ...
        for(int i = x + 1; i <= width; i++) {
            for(int j = y + 1; j <= height; j++) {
                point[j][i] = Math.max(point[j-1][i], point[j][i-1]) + ;
            }
        }
