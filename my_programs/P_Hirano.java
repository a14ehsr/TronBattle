...
public class P_Hirano {
    ...
    static final String playerName = "HIRANO"; // Do not include spaces in your name.
    ...
    public int put() {
        ...
        if(y < height && board[y+1][x] == NOT_ACHIEVED) {
            double tmp = DFS(x, y+1, x, y, 1, maxDepth, board);
        ...
        if(x < width && board[y][x+1] == NOT_ACHIEVED) {
            double tmp = DFS(x+1, y, x, y, 1, maxDepth, board);
    ...
    private double DFS(int x, int y, int beforeX, int beforeY, int depth, int maxDepth, int[][] board) {
        if(depth == maxDepth) {
            return 1;
        }
        ...
        } else if(y >= height) { // 壁の時
            patternCount -= exps[depth];
        } else {
